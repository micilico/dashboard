...
const S = {
  ...
  trash: [], autoRefreshTimer: null, uploadJobs: [], pendingLoad: false,
};
...
async function loadFiles(append) {
  if (S.loading) { if (!append) S.pendingLoad = true; return; }
  S.loading = true;
  ...
  S.loading = false;
  if (S.pendingLoad) { S.pendingLoad = false; loadFiles(false); }
}
...
async function moveItemsTo(destPath, items) {
  ...
  try {
    ...
    if (items.length === 1) {
      const item = items[0];
      fd.append("path", item.path.split("/").slice(0, -1).join("/"));
      fd.append("name", item.path.split("/").pop());
      result = await api(au("/files/move"), { method: "POST", body: fd });
    } else {
      fd.append("items", JSON.stringify(items.map(it => ({ path: it.path.split("/").slice(0, -1).join("/"), name: it.path.split("/").pop() }))));
      result = await api(au("/files/move-batch"), { method: "POST", body: fd });
    }
    moved = Number(result.moved) || items.length;
    ...
    toast(`Déplacement refusé : ${e.message}`);
  }
  S.selected.clear();
...
function renderBulkBar() {
  ...
  ["bulkMove", "bulkCopy", "bulkShare", "bulkDelete"].forEach(id => { $(id).hidden = n === 0; });
...
document.addEventListener("keydown", (e) => {
  if ($("uploadDialog").open || $("moveDialog").open || $("mkdirDialog").open || $("renameDialog").open || $("deleteDialog").open || $("shareDialog").open || $("previewDialog").open || $("organizeDialog").open || $("touchDialog").open || $("editDialog").open || $("propertiesDialog").open || $("bulkDeleteDialog").open) {
  ...
  if ((e.key === "c" || e.key === "x") && (e.ctrlKey || e.metaKey) && S.selected.size) {
    ...
    if (e.key === "x") cutToClipboard(sel); else copyToClipboard(sel);
  }
});
...
$("searchInput").addEventListener("input", () => {
  ...
  window.cloudSearchTimer = setTimeout(() => loadFiles(false), 650);
  updateUrl();
});
...
function cutToClipboard(items) {
  S.clipboard = { mode: "cut", items: items.map(i => ({ path: i.path, name: i.name, is_dir: Boolean(i.is_dir) })) };
  toast(`Coupé : ${items.length} élément${items.length > 1 ? "s" : ""}. Utilisez « Coller ici ».`);
  renderBulkBar();
}
...
async function pasteItems() {
  ...
  if (S.clipboard.mode === "cut") await moveItemsTo(S.path, items);
  else await copyItems(items, S.path);
  if (S.clipboard.mode === "cut") {
    S.clipboard = { mode: null, items: [] };
    renderBulkBar();
  }
}
...
$("bulkMove").addEventListener("click", () => {
  if (!S.selected.size) return;
  $("movePathInput").value = S.path;
  $("moveDescription").textContent = `${S.selected.size} élément${S.selected.size > 1 ? "s" : ""} sélectionné${S.selected.size > 1 ? "s" : ""}.`;
  $("moveMessage").textContent = "";
  $("moveDialog").showModal();
  $("movePathInput").focus();
});
$("cancelMoveBtn").addEventListener("click", () => $("moveDialog").close());
$("moveForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  const items = [...S.selected].map(p => S.selectedItems.get(p) || { path: p, name: p.split("/").pop(), is_dir: false });
  const dest = $("movePathInput").value.trim().replace(/^\/+|\/+$/g, "");
  setButtonBusy($("confirmMoveBtn"), true);
  try { await moveItemsTo(dest, items); $("moveDialog").close(); }
  catch (e) { $("moveMessage").textContent = e.message; }
  finally { setButtonBusy($("confirmMoveBtn"), false); }
});
