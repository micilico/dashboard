...
function seriesLineChart(daily, series, { height = 180, width = 440, tickEvery = 5 } = {}) {
  ...
  const xFor = (index) => index * stepX;
  const max = Math.max(1, ...points.map((item) => Math.max(...series.map((s) => Number(item[s.key]) || 0))));
  const yFor = (value) => padTop + (1 - (value || 0) / max) * plotHeight;
  ...
  series.forEach((s) => {
    ...
    const path = points
      .map((item, index) => `${xFor(index).toFixed(1)},${yFor(Number(item[s.key]) || 0).toFixed(1)}`)
      .join(" ");
    svg.append(svgEl("polyline", { points: path, class: `chart-line-path${className}` }));
    points.forEach((item, index) => {
      const dot = svgEl("circle", { cx: xFor(index), cy: yFor(Number(item[s.key]) || 0), r: 2.5, class: `chart-line-dot${className}` });
      const title = svgEl("title");
      title.textContent = `${item.date} · ${s.label}: ${s.format ? s.format(Number(item[s.key]) || 0) : Number(item[s.key]) || 0}`;
      dot.append(title);
      dotsByIndex[index].push(dot);
      svg.append(dot);
    });
  });
  ...
  const hit = svgEl("rect", { x: 0, y: padTop, width, height: plotHeight, class: "chart-hit" });
  ...
  const moveAt = (clientX) => {
    ...
    const index = Math.max(0, Math.min(points.length - 1, Math.round(xViewBox / stepX)));
    ...
    const rows = series.map((s) => {
      ...
      return `${s.label}: ${s.format ? s.format(value) : String(value)}`;
    });
    if (series.length > 1) rows.push(`Ratio: ${points[index].ratio ?? 0}`);
    tooltip.replaceChildren(
      Object.assign(document.createElement("strong"), { textContent: shortDate(points[index].date) }),
      ...rows.map((rowText) => Object.assign(document.createElement("span"), { textContent: rowText })),
    );
    tooltip.hidden = false;

    const tipWidth = tooltip.getBoundingClientRect().width;
    let left = pointX + 12;
    if (left + tipWidth > boxRect.width) left = pointX - tipWidth - 12;
    tooltip.style.left = `${Math.max(4, left)}px`;
    let top = pointY - tooltip.getBoundingClientRect().height - 10;
