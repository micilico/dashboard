...
const els = {
  ...
  storageVisualization: document.querySelector("#storageVisualization"),
...
function setView(view, { push = true } = {}) {
  ...
  els.summaryGrid.hidden = state.activeView !== "torrents";
...
function renderServices() {
  ...
  const orderedNames = ["Jellyfin", "qBittorrent", "Prowlarr", "rclone"];
  ...
  els.servicesSummary.textContent = `${operational}/${services.length} service(s) operationnel(s).`;
...
function renderOverviewMetrics() {
  ...
  const totalIndexers = Number(overview.indexersTotal || 0);
  const activeIndexers = Number(overview.indexersActive || 0);
  ...
  pushMetricHistory("indexersRatio", totalIndexers ? activeIndexers / totalIndexers : 0);
  const cards = [
    {
      label: "Débit",
    ...
    {
      label: "Indexeurs",
      value: totalIndexers ? `${activeIndexers}/${totalIndexers}` : "—",
      meta: overview.indexersError ? `${overview.indexersError} en erreur` : "Tous synchronisés",
      icon: "indexers",
      historyKey: "indexersRatio",
    },
  ];
...
function renderStorageCard() {
  const disk = state.storage.disk || state.dashboard.storage || {};
  const totalBytes = Number(disk.totalBytes || 0);
  const usedBytes = Number(disk.usedBytes || 0);
  const freeBytes = Number(disk.freeBytes || 0);
  const usedPercent = Math.max(0, Math.min(100, Number(disk.usedPercent || 0)));
  els.storageSummary.textContent = disk.mounted === false
    ? "Le volume surveillé est actuellement indisponible."
    : "Capacité et occupation du stockage surveillé.";
  const trackPercent = Math.max(0.08, usedPercent / 100);
  els.storageVisualization.innerHTML = `
    <div class="orbit-scene" aria-hidden="true">
      <svg viewBox="0 0 420 300" preserveAspectRatio="xMidYMid meet">
        <defs>
          <linearGradient id="orbitGlow" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#a496ff"></stop>
            <stop offset="100%" stop-color="#6d5cff"></stop>
          </linearGradient>
          <radialGradient id="planetFill" cx="35%" cy="30%" r="70%">
            <stop offset="0%" stop-color="#9f97ff"></stop>
            <stop offset="45%" stop-color="#5a51c8"></stop>
            <stop offset="100%" stop-color="#161935"></stop>
          </radialGradient>
        </defs>
        <ellipse cx="210" cy="155" rx="140" ry="42" class="orbit orbit-outer"></ellipse>
        <ellipse cx="210" cy="148" rx="108" ry="30" class="orbit orbit-mid"></ellipse>
        <ellipse cx="210" cy="143" rx="76" ry="21" class="orbit orbit-inner"></ellipse>
        <path d="M 84 170 A 140 42 0 0 0 336 170" class="orbit-progress" style="--orbit-progress:${trackPercent};"></path>
        <circle cx="210" cy="140" r="46" fill="url(#planetFill)"></circle>
        <circle cx="132" cy="132" r="5" class="orbit-satellite"></circle>
        <circle cx="294" cy="112" r="6" class="orbit-satellite"></circle>
        <circle cx="170" cy="230" r="5" class="orbit-satellite orbit-satellite-soft"></circle>
      </svg>
    </div>
    <div class="storage-copy">
      <strong class="storage-value">${disk.mounted === false ? "Indisponible" : formatStorageValue(freeBytes)}</strong>
      <span class="storage-label">${disk.mounted === false ? "volume non monté" : "disponibles"}</span>
      <div class="storage-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${usedPercent.toFixed(1)}">
        <span style="width:${usedPercent}%"></span>
      </div>
      <p class="storage-meta">${formatStorageValue(usedBytes)} utilisés / ${formatStorageValue(totalBytes)} au total</p>
    </div>
  `;
}
...
function renderHome() {
  ...
  if (greeting) greeting.textContent = "Bonsoir Corentin";
  if (els.statusText) {
